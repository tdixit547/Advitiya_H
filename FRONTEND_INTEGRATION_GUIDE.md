...
**Description**: Get Sankey diagram data for visualizing user flow (source → link → destination).
...
|-----------|------|-------------|
| `start_date` | string | ISO date string for period start |
| `end_date` | string | ISO date string for period end |
...
```json
{
  ...
  "links": [
    ...
    { "source": "direct", "target": "variant-001", "value": 80 },
    { "source": "variant-001", "target": "dest_youtube.com", "value": 230 }
  ],
  "period": {
...
**Description**: Get Network graph data with nodes (weighted by interactions) and edges (transitions).
