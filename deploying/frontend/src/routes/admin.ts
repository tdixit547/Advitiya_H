...
// ============== FLOW ANALYTICS (SANKEY & NETWORK GRAPHS) ==============

/**
 * Get Sankey diagram data for a hub
 * GET /hubs/:hub_id/analytics/sankey
 * Requires authentication and ownership
 * 
 * Query params:
 ...
 * 
 * Returns nodes (sources, links, destinations) and links (flows between them)
 */
router.get('/hubs/:hub_id/analytics/sankey', requireAuth, requireOwnership, async (req: IAuthenticatedRequest, res: Response) => {
    try {
        const { hub_id } = req.params;
        ...
        const sankeyData = await flowAnalyticsService.getSankeyData(hub_id, startDate, endDate);

        return res.json(sankeyData);
    } catch (error) {
        console.error('Get Sankey data error:', error);
        return res.status(500).json({ error: 'Failed to get Sankey diagram data' });
    }
});

/**
 * Get Network graph data for a hub
 * GET /hubs/:hub_id/analytics/network
 * Requires authentication and ownership
 * 
 * Query params:
 * - start_date: ISO date string (optional)
 * - end_date: ISO date string (optional)
 * 
 * Returns nodes (with weights) and edges (transitions between nodes)
 */
router.get('/hubs/:hub_id/analytics/network', requireAuth, requireOwnership, async (req: IAuthenticatedRequest, res: Response) => {
    try {
        const { hub_id } = req.params;
        const { start_date, end_date } = req.query;
        ...
        const networkData = await flowAnalyticsService.getNetworkData(hub_id, startDate, endDate);

        return res.json(networkData);
    } catch (error) {
        console.error('Get Network data error:', error);
        return res.status(500).json({ error: 'Failed to get Network graph data' });
    }
});
